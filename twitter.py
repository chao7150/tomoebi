...
class StreamListener(tweepy.StreamListener):
    ...
    def __init__(self, name, callback, RTfilter):
...
def open_userstream(api, callback, name):
    ...
    """
    stream = tweepy.Stream(auth=api.auth, listener=StreamListener(name, callback, False))
    stream.userstream(async=True)
    return stream

def open_filterstream(api, callback, name, query):
    stream = tweepy.Stream(auth=api.auth, listener=StreamListener(name, callback, True))
